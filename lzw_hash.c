...
int lzw(unsigned char *in_chunk, unsigned char *out_chunk, int chunk_length, int * compressed_chunk_lenghth)
{
    ...
    while (index != chunk_length) {
        ...
        //printf("prefix=%c, c=%c\n",prefix,c);
        //printf("%d iteration, prefix=%d, c=%c\n",p,prefix,c);
        ...
        //printf("256 is %c,%c\n",dic[256].prefix_code,dic[256].suffix_char);
        if (i != -1){
            prefix = i;
        //printf("prefix=%d c= %c\n",prefix,c);
        }
...
{
    int ret=-1;
    long temp = prefix + (long)character;
    ...
    else if((dic[address].prefix_code==-3) && (dic[address].suffix_char==(unsigned char)(-3))){
        //dic[address].prefix_code=prefix;
        //dic[address].suffix_char=character;
        //DicEntryNumber++;
        ret = -1;
    ...
    //don't find & the address isn't empty
        ...
        while(k<(MAX_DICT_SIZE+address)){
            ...
            else if((dic[rehash].prefix_code==-3) && (dic[rehash].suffix_char==(unsigned char)(-3))){
                ret= -1;
                break;
            }
            else{
...
void dictionary_add(int prefix, unsigned char character)
{
    long temp = prefix + (long)character;
    int address = temp % 4093;
    int k=address;
    ...
    while(k<(MAX_DICT_SIZE+address)){
        rehash=k%MAX_DICT_SIZE;
        if((dic[rehash].prefix_code==-3) && (dic[rehash].suffix_char==(unsigned char)(-3))){
